...
Code review must reject any refactor PR that violates this contract — see the **Code Review Application** section below for the enforcement rules.
...
- **Enforce the Test Coverage Contract above on every refactor PR.** Walk the PR commit history and verify: (a) the refactor commit is preceded by a `test(scope): ...` commit that brings the refactored lines to 100% coverage, (b) no pre-existing test file is modified inside the refactor commit (mechanical-rename exemption only, and only when the commit body documents it), (c) coverage of the refactor commit alone — verified with the project's available coverage tooling scoped to the changed files — reports 100% for changed lines. Missing pre-refactor coverage commit, modified tests inside the refactor commit, or sub-100% coverage on the changed lines are each **Critical** findings.
